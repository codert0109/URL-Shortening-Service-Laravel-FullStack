...
class MainController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('shortened_url', function ($row) {
                    $shortened_url = env('DOMAIN_URL') . $row['slug'];
                    return $shortened_url;
                })
                ->editColumn('destination', function ($row) {
                    $destination = $row['destination'];
                    if (strlen($destination) > 30) {
                        $destination = substr($row['destination'], 0, 30) . '...';
                    }
                    return $destination;
                })
                ->addColumn('action', function ($row) {
                    $actionBtn = '<a id="' . $row->id . '" class="delete btn btn-danger btn-sm text-white">Delete</a>';
                    return $actionBtn;
                })
                ->rawColumns(['action'])
                ->make(true);
    ...
    public function delete($id)
    {
        ...
        return response()
            ->json([
                'status' => 'Error',
                'message' => 'Cannot delete the url, try again!',
            ], 500);
    }
}
